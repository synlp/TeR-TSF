...
import numpy as np
import torch
from Models.MCD_TSF.data_provider.data_factory import data_provider
...
def get_dataloader(datatype,device,batch_size=8,args=None):
    ...
    if datatype == 'multimodal':
        ...
        valid_dataset, valid_loader = data_provider(args, flag='val')
        test_dataset, test_loader = data_provider(args, flag='test')
    else:
        dataset = Forecasting_Dataset(datatype,mode='train')
        valid_dataset = Forecasting_Dataset(datatype,mode='val')
