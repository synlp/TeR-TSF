...
from Models.MCD_TSF.diff_models import diff_CSDI
from Models.MCD_TSF.utils.prepare4llm import get_llm
...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device, window_lens, freq):
        super().__init__()
        self.device = device
        ...
        self.lookback_len = window_lens[0]
        self.pred_len = window_lens[-1]
        ...
        self.llm = "bert"
        config["model"]["timestep_dim"] = timestep_dim_dict[freq]
...
class CSDI_Forecasting(CSDI_base):
    def __init__(self, config, device, target_dim, window_lens, freq, guide_w=-1):
        super(CSDI_Forecasting, self).__init__(target_dim, config, device, window_lens, freq)
    ...
    def evaluate(self, batch, n_samples, guide_w=-1):
        (
            observed_data,
            observed_mask,
            observed_tp,
