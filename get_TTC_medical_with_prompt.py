import pandas as pd
import argparse
from tqdm import tqdm
import os
import re
from glob import glob

main_prompt_template = """
Analyze the following time series data and provide insights that could help with forecasting.

Historical data:
{data_records}

Statistics:
{statistical_summary}

Future prediction period:
{future_dates}

Provide your analysis.
"""


data_record_template = "In {date}, the {feature_name} is {value:.2f}, the report is: {report}"
statistical_summary_template = "Statistical summary:\n- Maximum: {max_val}\n- Minimum: {min_val}\n- Average: {avg_val}\n- Standard deviation: {std_val}"


def construct_prompt(window_df, future_df, feature_name="value", var_name="Heart_Rate"):
    records = []
    for _, row in window_df.iterrows():
        report = row["text"] if pd.notna(row["text"]) and row["text"].strip() != "" else "(no text)."
        record = data_record_template.format(
            date=pd.to_datetime(row["date"]),
            feature_name=feature_name,
            value=row[var_name],
            report=report
        )
        records.append(record)
    data_records = "\n".join(records)

    values = window_df[var_name]
    stats = statistical_summary_template.format(
        max_val=round(values.max(), 2),
        min_val=round(values.min(), 2),
        avg_val=round(values.mean(), 2),
        std_val=round(values.std(), 2)
    )

    # 只取第一个和最后一个日期作为预测范围
    start_date = pd.to_datetime(future_df["date"].iloc[0])
    end_date = pd.to_datetime(future_df["date"].iloc[-1])
    future_dates = f"From {start_date} to {end_date}"

    prompt = main_prompt_template.format(
        data_records=data_records,
        statistical_summary=stats,
        future_dates=future_dates
    )

    return prompt

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--csv_file_dir", type=str, help="CSV file with Data, Value, Text columns", default='/home/suchen/Multimodal_Forecasting-main/data')
    parser.add_argument("--var_name", type=str, help="variable of time series", default='Heart_Rate')
    parser.add_argument("--hist_len", type=int, help="Historical time series length", default=4)
    parser.add_argument("--pred_len", type=int, help="Prediction length", default=4)
    parser.add_argument("--save_dir", type=str, help="save path of processed dataset", default='./datasets_with_prompt')
    
    args = parser.parse_args()

    data_dir = os.path.join(args.csv_file_dir, 'medical')
    var_name = args.var_name
    csv_list = glob(data_dir+'/*.csv')
    num_train = int(len(csv_list) * 0.7)
    num_test = int(len(csv_list) * 0.2)
    train_csv_path_list = csv_list[:num_train]
    val_csv_path_list = csv_list[num_train:len(csv_list)-num_test]
    test_csv_path_list = csv_list[len(csv_list)-num_test:]

    dataset_list = {'train': train_csv_path_list, 'val': val_csv_path_list, 'test': test_csv_path_list}
    
    for dataset_type in dataset_list.keys():
        new_df = []
        for data_path in dataset_list[dataset_type]:
            df = pd.read_csv(data_path)
            df = df.dropna(subset=["date", var_name]).reset_index(drop=True)
            for start in tqdm(range(0, len(df) - args.hist_len - args.pred_len + 1)):
                hist_df = df.iloc[start:start + args.hist_len]
                pred_df = df.iloc[start + args.hist_len:start + args.hist_len + args.pred_len]

                prompt = construct_prompt(hist_df, pred_df, feature_name='value', var_name=var_name)

                original_texts = hist_df["text"].dropna(how='any', axis=0).tolist()
                if original_texts:
                    original_texts = " ".join(original_texts)
                    if len(original_texts) > 5120:
                        original_texts = original_texts[:5120]
                else:
                    original_texts = "No text."

                
                new_df.append({
                    "history_series": hist_df[var_name].tolist(),
                    "prompt": prompt,
                    "horizon_series": pred_df[var_name].tolist(),
                    "original_text": original_texts
                })

        output_df = pd.DataFrame(new_df)
        os.makedirs(args.save_dir, exist_ok=True)
        output_path = os.path.join(args.save_dir+'/'+f'{var_name}_{args.hist_len}_{args.pred_len}_'+dataset_type+'.csv')
        output_df.to_csv(output_path, index=False)

        print(f"Saved results to {output_path}")


if __name__ == "__main__":
    main()
