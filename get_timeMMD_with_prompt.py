...
def main():
    ...
    parser.add_argument("--csv_file_dir", type=str, help="CSV file with Data, Value, Text columns", default='./timeMMD_processed')
    parser.add_argument("--domain", type=str, help="name of domain", default='Agriculture')
    ...
    data_path = data_dir + '/' + args.domain + '.csv'
    df_all = pd.read_csv(data_path)
    df_all = df_all.dropna(subset=["date", "value"]).reset_index(drop=True)
    ...
    for dataset_type in type_map.keys():
        ...
        for start in tqdm(range(0, len(df) - args.hist_len - args.pred_len + 1)):
            ...
            new_df.append({
                "history_series": hist_df["value"].tolist(),
                "prompt": prompt,
                "horizon_series": pred_df["value"].tolist(),
                "original_text": original_texts
            })

        output_df = pd.DataFrame(new_df)
        os.makedirs(args.save_dir, exist_ok=True)
        output_path = os.path.join(args.save_dir+'/'+args.domain+f'_{args.hist_len}_{args.pred_len}_'+dataset_type+'.csv')
