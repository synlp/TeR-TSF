...
def get_desc(domain, lookback_len, pred_len):
    ...
    description = {
        ...
        "ETTh1": ["Oil Temperature", "hour"],
        "exchange_rate": ["Exchange rate", "daily"],
        "weather": ["Temperature", "daily"],
        "medical": ["Heart Rate", "daily"]
    }
...
def get_llm(llm_model:str, llm_layers:int=0):
    if llm_model == 'llama':
        ...
        bert_config = BertConfig.from_pretrained('/media/ubuntu/data/share/bert-base-uncased')
        if llm_layers:
        ...
        try:
            llm_model = BertModel.from_pretrained(
                '/media/ubuntu/data/share/bert-base-uncased',
                local_files_only=True,
                config=bert_config,
            )
        except EnvironmentError:  # downloads model from HF is not already done
